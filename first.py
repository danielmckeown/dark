...
progenitors = ([])
dark_mass_percent = ([])
subs = get( snap['subhalos'], {'limit':300} )

#####  HERE WE SET THE NUMBER OF HALOS COUNTED
subs = get( snap['subhalos'], {'limit':300, 'order_by':'-mass_dm'} )

c =  [ subs['results'][i]['id'] for i in range(300) ]
...
while n < len(c):
	...
	with h5py.File(saved_filename) as f:
		...
		coun = 0
		iter = len(rrr2) / (5000)
		remainder = len(rrr2) % (5000)
		dark_matters = ([])
		outer_radius2 = ([])
		while coun < iter:
			top5 = rrr2[:5000]
			outer_radius2.insert(coun,top5[4999])
